...
def api_key():
    ...
    if not api_key:
        raise ValueError("TL_API_KEY 환경 변수가 설정되지 않았습니다. config.env 파일을 확인하거나 환경 변수를 설정하세요.")
    return api_key
...
def index_id(request):
    ...
    if hasattr(request, 'param') and isinstance(request.param, str) and request.param.startswith("index_"):
        return request.getfixturevalue(request.param)
    
    # 기본 동작: 환경 변수에서 가져오기
    index_id = os.getenv("TL_INDEX_ID")
    if not index_id:
        raise ValueError("TL_INDEX_ID 환경 변수가 설정되지 않았습니다. config.env 파일을 확인하거나 환경 변수를 설정하세요.")
    return index_id
...
def index_marengo27():
    ...
    if not index_id:
        pytest.skip("TL_INDEX_MARENGO_27 환경 변수가 설정되지 않았습니다. config.env 파일을 확인하거나 환경 변수를 설정하세요.")
    return index_id
...
def index_marengo30():
    ...
    if not index_id:
        pytest.skip("TL_INDEX_MARENGO_30 환경 변수가 설정되지 않았습니다. config.env 파일을 확인하거나 환경 변수를 설정하세요.")
    return index_id
...
def validate_marengo_fields(item, index_name: str = None, request=None):
    ...
    if is_30:
        # Marengo 3.0 필드
        assert item.video_id is not None, f"video_id는 필수입니다 (Marengo 3.0, index: {index_name})"
        assert item.rank is not None, f"rank는 필수입니다 (Marengo 3.0, index: {index_name})"
        assert isinstance(item.rank, int), f"rank는 정수여야 합니다 (Marengo 3.0, index: {index_name})"
        assert item.rank > 0, f"rank는 1 이상이어야 합니다 (Marengo 3.0, index: {index_name})"
        # transcription은 optional이므로 None일 수 있음
    else:
        # Marengo 2.7 필드
        assert item.score is not None, f"score는 필수입니다 (Marengo 2.7, index: {index_name})"
        assert item.confidence is not None, f"confidence는 필수입니다 (Marengo 2.7, index: {index_name})"
        assert isinstance(item.score, (int, float)), f"score는 숫자여야 합니다 (Marengo 2.7, index: {index_name})"
